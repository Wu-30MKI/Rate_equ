%% 
%====== 速率方程包含噪声，不包括外光注入 对应方程 rate_equ_noise ===========
%====== 仿真 无外光注入下的 静态特性 和 动态特性 ==========================
%====== 通过改变 自发辐射因子beta，得到主激光器的参数======================
% clc 
% clear
% clf
% close all
...
[t,y] = ode23(@(t,y) rate_equ_switch(t,y,I0,Im,f,on,Bsim),tspan, y0);   %包含噪声 龙格库塔 解微分方程 
...
E = sqrt(Sm).*exp(1i*Phim);%E = sqrt(Sm).*exp(1i*2*pi*5*1e9*t).*exp(1i*Phim)
OS =abs( fftshift(fft(ifftshift(sqrt(K)*E))) );

maxa = max(OS);
A = 10*log10(OS./maxa);

figure,plot(f0/1e9,A);
...
S_m = Sm-Pavg;
FFT_S = fft(S_m);
unique = ceil( (length(Sm)+1)/2 );
FFT_S = abs( FFT_S(1:unique) )*2;
FFT_S(1) = FFT_S(1)/2;
FFT_S(length(FFT_S)) = FFT_S(length(FFT_S))/2;
FFT_S = FFT_S/length(Sm);

f = (0:unique-1)*n;
% drop 0 freq point.为什么？
FFT_S = FFT_S(2:end);
f = f(2:end);
% % RIN power: <i(f)>^2. Average current a frequency, then squared.
% % RIN = FFT of photons - average photons - normalization to /Hz
RIN = 20*log10(FFT_S) - 20*log10(Pavg) - 10*log10(Fs/unique);
RIN = smooth(RIN,0.001);
figure,plot(f/1e9,RIN);
set(gca,'XScale','log')
xlim([10^-1 10^2]);
% 
% % total noise
% specdata = 20*log10(FFT_S) - 10*log10(Fs/unique);
% global fr_index
% if ~size(fr_index)
%    t=find ( specdata==max(specdata) ); 
%    fr_index=t(1);
%    fr=f(fr_index);
% else
%    f(fr_index)
% end;
% spec_tot = 20*log10(sum ( smooth(FFT_S(1:fr_index),0.003) ) ); % total noise
% ...belowresonance frequency, over total bandwidth
% RIN_tot = spec_tot - 2*10*log10(Pavg);
